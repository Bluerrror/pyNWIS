...
### Fixed
- `fetch_batch_usgs_data` now returns an empty DataFrame with correct column headers when no sites match the filter (previously returned a completely empty DataFrame causing `df.describe()` to crash)
- Updated README example 3 to use a working site/parameter combination
